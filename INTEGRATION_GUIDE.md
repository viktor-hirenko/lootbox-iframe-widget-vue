...
- ⚠️ A/B тестування: Якщо для проекту налаштовано A/B тест і не вказано явно `?style=`, тема буде обрана автоматично на основі варіанту користувача
...
2. Якщо передано `?project=` без `?style=`:
...
3. Якщо передано обидва `?project=` + `?style=` — валідується належність теми до проекту, A/B тест вимкнено
